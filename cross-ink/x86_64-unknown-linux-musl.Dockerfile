...
RUN --mount=type=bind,source=./cross-ink/scripts/install-clang.sh,target=/run.sh /run.sh

# Install musl
RUN --mount=type=bind,source=./cross-ink/scripts/install-musl.sh,target=/run.sh /run.sh

# Openssl
ENV OPENSSL_INCLUDE_DIR=/usr/local/$CROSS_TOOLCHAIN/include
ENV OPENSSL_LIB_DIR=/usr/local/$CROSS_TOOLCHAIN/lib
RUN --mount=type=bind,source=./cross-ink/scripts/install-openssl-musl.sh,target=/run.sh /run.sh
